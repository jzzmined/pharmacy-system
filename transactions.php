...
try {
    ...
    $s = $db->prepare("
        SELECT
            ...
            pr.PrescriptionID,
            i.PharmacistID,
            SUM(pd.Quantity)            AS DispenseQty,
            AVG(md.UnitPrice)           AS UnitPrice,
            IFNULL(SUM(pd.Discount), 0) AS Discount,
            i.Subtotal,
            i.Total,
            i.Status,
            pr.DatePrescribed
        ...
        JOIN medicationdetails md   ON pd.MedicationDetailID = md.MedicationDetailID
